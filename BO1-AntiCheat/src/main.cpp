...
//int main()
{
	...
	// watermark text
	sf::Text watermarkText("Created with <3 by Evelyn", semi_bold_font, 15);
	watermarkText.setFillColor(sf::Color::White);
	watermarkText.setPosition((main_window_width - watermarkText.getGlobalBounds().width) / 2, 105);
		...
		while (updates_window.isOpen())
		{
			...
			while (updates_window.pollEvent(event))
			{
				...
				if (event.type == sf::Event::MouseButtonPressed && event.mouseButton.button == sf::Mouse::Left)
				{
					...
					{
						ShellExecute(NULL, L"open", L"https://github.com/BlackOpsOne/BO1-AntiCheat/releases/latest", NULL, NULL, SW_SHOWNORMAL);
			...
			// hover effect for the ignore button
	...
	if (anticheat::utils::strings::ToLower(display_guidelines) == "true")
	{
		...
		std::string guidelines = "The following guidelines are important to follow to ensure integrity of the tool.\n\n" + Constants::GUIDELINES + "\n\nYou can stop this from appearing by editing the config file.";
		std::string wrapped_guidelines = WrapText(guidelines, semi_bold_font, 15, guidelines_window_width - 30);
		...
		// guidelines text
		sf::Text guidelinesText(wrapped_guidelines, semi_bold_font, 15);
	...
	while (main_window.isOpen())
	{
		...
		anticheatText.setPosition(((main_window_width - anticheatTextWidth) / 2) + padding, 15);
		bo1Sprite.setPosition(anticheatText.getGlobalBounds().left - (padding + 19), 12);
		...
		statusText.setPosition((main_window_width - statusTextWidth) / 2, 47); // adjust position based on text width and main_window width
